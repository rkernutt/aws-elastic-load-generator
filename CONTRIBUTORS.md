# Contributors

Thanks to everyone who contributes to this project.

## Development

- **Cursor** ([cursor.com](https://cursor.com)) — AI-assisted development (code generation, refactoring, and documentation) was used to build and maintain this project. Listed here for transparency.
- **Claude** ([claude.ai](https://claude.ai)) — AI-assisted development (code review, modular refactoring, feature additions, and documentation) contributed from v7.1.2 onwards. Listed here for transparency.
- **Project maintainer** — See the repository owner / commit history for primary authorship and maintenance.

---

_If you contribute code, docs, or ideas, you can add yourself to this file in a pull request._
