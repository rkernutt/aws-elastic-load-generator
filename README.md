...
- **Metrics mode & packaging alignment** — `package.json` `version` is **11.2.0** (matches documented releases). **Metrics** mode now lists all **150** services that have dimensional metric generators (App Mesh, Client VPN, Cloud Map, Outposts, Verified Permissions, DAX, Chime SDK Voice, Elastic DRS were missing from the selector set and are restored). README usage and configuration counts use **185** logs / **150** metrics / **20** traces.
- **`"type": "module"`** — The package is explicitly **ESM**; root **`proxy.js`** is renamed to **`proxy.cjs`** (CommonJS) so the bulk proxy keeps working under `require` in Docker and local `node proxy.cjs`. **`npm run samples:verify`** checks that sample files exist for every log, metric, and trace generator.
