...
- **`"type": "module"`** — The package is explicitly **ESM**; the bulk proxy is **`proxy.cjs`** (CommonJS) so it runs with `require` in Docker and via `node proxy.cjs` locally. **`npm run samples:verify`** checks that sample files exist for every log, metric, and trace generator.
