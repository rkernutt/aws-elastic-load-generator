...
export function Card({children, style={}}) {
  return (
    <div style={{background:K.plain,border:`1px solid ${K.border}`,borderRadius:K.radius,padding:16,boxShadow:K.shadow,...style}}>
      {children}
    </div>
  );
}
...
export function QuickBtn({children, onClick}) {
  return (
    <button onClick={onClick} style={{fontSize:11,padding:"4px 10px",borderRadius:K.radiusSm,border:`1px solid ${K.borderPlain}`,background:K.subdued,color:K.textSubdued,cursor:"pointer",fontFamily:"inherit",fontWeight:500}}>
      {children}
    </button>
  );
}
...
export function Field({label, children}) {
  return (
    <div>
      <div style={{fontSize:12,fontWeight:500,color:K.textSubdued,marginBottom:6}}>{label}</div>
      {children}
    </div>
  );
}

export function SliderField({label, value, min, max, step, onChange, display, sublabel}) {
  return (
    <div>
...
export function StatCard({label, value, color}) {
  return (
    <div style={{background:K.subdued,border:`1px solid ${K.border}`,borderRadius:K.radiusSm,padding:"10px 12px"}}>
