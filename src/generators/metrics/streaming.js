...
const SQS_QUEUES = [
  "orders-queue","notifications-queue","email-queue","payment-events","audit-queue",
  "dlq-orders","dlq-notifications","batch-jobs","scheduled-tasks","webhook-delivery",
];
...
export function generateSqsMetrics(ts, er) {
  ...
  return sample(SQS_QUEUES, randInt(2, 6)).map(queue => {
    ...
    return metricDoc(ts, "sqs", "aws.sqs", region, account,
      { QueueName: queue },
      {
        ...
        ApproximateAgeOfOldestMessage:     stat(isDLQ && Math.random() < er ? jitter(7200, 5000, 100, 86400) : jitter(30, 20, 0, 300)),
...
const KINESIS_STREAMS = [
  "clickstream","transaction-events","user-activity","sensor-data",
  "log-stream","audit-trail","order-events","payment-stream",
];
...
export function generateKinesisMetrics(ts, er) {
  ...
  return sample(KINESIS_STREAMS, randInt(2, 4)).map(stream => {
    ...
    const iterAge = Math.random() < er ? jitter(3_600_000, 2_000_000, 100_000, 86_400_000) : jitter(5_000, 4_000, 0, 60_000);
    return metricDoc(ts, "kinesis", "aws.kinesis", region, account,
      { StreamName: stream, ShardId: `shardId-00000000000${randInt(0, shards - 1)}` },
      {
        "GetRecords.Bytes":           counter(Math.round(bytesIn * 0.9)),
        "GetRecords.IteratorAgeMilliseconds": stat(dp(iterAge), { max: dp(iterAge * jitter(3, 1.5, 1.5, 10)) }),
...
const MSK_TOPICS   = ["orders","payments","user-events","clickstream","notifications","audit","metrics","logs"];
...
export function generateMskMetrics(ts, er) {
  ...
  return sample(MSK_TOPICS, randInt(2, 5)).map(topic => {
    return metricDoc(ts, "kafka", "aws.kafka_metrics", region, account,
...
const SNS_TOPICS = [
  "order-notifications","payment-alerts","system-alerts","user-signups",
  "dlq-notifications","push-notifications","email-delivery","sms-delivery",
];
...
export function generateSnsMetrics(ts, er) {
  ...
  return sample(SNS_TOPICS, randInt(2, 5)).map(topic => {
    ...
    const failed    = Math.round(published * (Math.random() < er ? jitter(0.05, 0.04, 0.001, 0.3) : jitter(0.001, 0.0008, 0, 0.005)));
    return metricDoc(ts, "sns", "aws.sns", region, account,
      { TopicName: topic },
      {
...
const FIREHOSE_STREAMS = [
  "clickstream-to-s3","logs-to-opensearch","events-to-redshift",
  "metrics-to-s3","audit-to-splunk","traces-to-s3",
];
...
export function generateFirehoseMetrics(ts, er) {
  ...
  return sample(FIREHOSE_STREAMS, randInt(1, 3)).map(stream => {
    ...
    return metricDoc(ts, "firehose", "aws.firehose", region, account,
      { DeliveryStreamName: stream },
      {
        ...
        DeliveryToS3_Success:    stat(dp(Math.random() < er ? jitter(85, 10, 50, 100) : jitter(99.9, 0.08, 99, 100))),
        DeliveryToS3_DataFreshness: stat(dp(jitter(60, 45, 5, 900))),
        KinesisMillisBehindLatest: stat(dp(Math.random() < er ? jitter(300_000, 200_000, 10_000, 3_600_000) : jitter(5_000, 4_000, 0, 30_000))),
...
export function generateEventbridgeMetrics(ts, er) {
  ...
  return sample(EB_BUSES, randInt(1, 3)).map(bus => {
    ...
    return metricDoc(ts, "events", "aws.eventbridge", region, account,
      { EventBusName: bus },
      {
...
export function generateAmazonmqMetrics(ts, er) {
  ...
  return [metricDoc(ts, "amazonmq", "aws.amazonmq", region, account,
    { Broker: brokerId, Queue: rand(["orders","payments","notifications","dlq"]) },
    {
      ...
      CPUCreditBalance:      stat(dp(Math.random() < er ? jitter(20, 15, 0, 50) : jitter(200, 100, 50, 576))),
...
export function generateKinesisanalyticsMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "kinesisanalytics", "aws.kinesisanalytics", region, account,
    { Application: rand(["anomaly-detection","stream-aggregator","real-time-etl","fraud-scorer"]) },
    {
      ...
      MillisBehindLatest:  stat(dp(Math.random() < er ? jitter(60_000, 50_000, 1_000, 3_600_000) : jitter(500, 400, 0, 5_000))),
    }
  )];
}
