...
import { REGIONS, ACCOUNTS, rand, randInt, dp, stat, counter, metricDoc, pickCloudContext, jitter, sample } from "./helpers.js";
...
export function generateWafMetrics(ts, er) {
  ...
  const rule = rand(["RateLimitRule","SQLiRule","XSSRule","GeoBlockRule","BadBotsRule","IPBlocklist"]);
  const req  = randInt(1_000, 5_000_000);
  const blocked = Math.round(req * (Math.random() < er ? jitter(0.1, 0.08, 0.01, 0.5) : jitter(0.02, 0.015, 0.001, 0.1)));
  return [metricDoc(ts, "waf", "aws.waf", region, account,
...
export function generateShieldMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "shield", "aws.shield", region, account,
    { ResourceArn: `arn:aws:elasticloadbalancing:${region}:${account.id}:loadbalancer/app/prod-alb/abc123` },
    {
      DDoSAttackBitsPerSecond:   stat(Math.random() < er ? dp(jitter(10_000_000, 8_000_000, 1_000, 1_000_000_000)) : 0),
      DDoSAttackPacketsPerSecond: stat(Math.random() < er ? dp(jitter(50_000, 40_000, 1_000, 10_000_000)) : 0),
      DDoSAttackRequestsPerSecond: stat(Math.random() < er ? dp(jitter(10_000, 8_000, 100, 1_000_000)) : 0),
...
const KMS_KEY_IDS = ["mrk-aabbccddeeff0011","mrk-1122334455aabbcc","key-99887766aabbccdd","key-aabbccddeeff1122"];

export function generateKmsMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return sample(KMS_KEY_IDS, randInt(1, 3)).map(keyId => {
    return metricDoc(ts, "kms", "aws.kms", region, account,
      { KeyId: keyId },
      {
...
export function generateCognitoMetrics(ts, er) {
  ...
  return sample(COGNITO_POOLS, randInt(1, 2)).map(pool => {
    ...
    return metricDoc(ts, "cognito", "aws.cognito", region, account,
      { UserPool: pool },
      {
        SignInSuccesses:          counter(Math.round(signIns * jitter(0.95, 0.04, 0.7, 1))),
        SignInThrottles:          counter(Math.random() < er ? randInt(1, 500) : 0),
        SignInUserErrors:         counter(Math.round(signIns * (Math.random() < er ? jitter(0.08, 0.06, 0.005, 0.3) : jitter(0.02, 0.015, 0.001, 0.05)))),
...
export function generateGuarddutyMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "guardduty", "aws.guardduty", region, account,
...
const CW_NAMESPACES = ["AWS/Lambda","AWS/EC2","AWS/RDS","AWS/DynamoDB","AWS/SQS","AWS/Kinesis","Custom/App"];

export function generateCloudwatchMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return sample(CW_NAMESPACES, randInt(2, 4)).map(ns => {
    return metricDoc(ts, "cloudwatch", "aws.cloudwatch_metrics", region, account,
...
const SF_MACHINES = [
  "order-fulfillment","payment-processing","data-pipeline","etl-workflow",
  "user-onboarding","notification-orchestrator","approval-workflow",
];
...
export function generateStepfunctionsMetrics(ts, er) {
  ...
  return sample(SF_MACHINES, randInt(1, 3)).map(machine => {
    ...
    const failed   = Math.round(started * (Math.random() < er ? jitter(0.05, 0.04, 0.001, 0.3) : jitter(0.005, 0.004, 0, 0.02)));
    const aborted  = Math.round(started * (Math.random() < er * 0.3 ? jitter(0.02, 0.015, 0, 0.1) : 0));
    return metricDoc(ts, "states", "aws.states", region, account,
...
export function generateSsmMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "ssm", "aws.ssm", region, account,
    { Region: region },
    {
...
export function generateCloudformationMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "cloudformation", "aws.cloudformation", region, account,
    { Region: region },
    {
...
const CODEBUILD_PROJECTS = ["api-service-build","frontend-ci","mobile-build","docker-image-build","terraform-validate"];
...
export function generateCodebuildMetrics(ts, er) {
  ...
  return sample(CODEBUILD_PROJECTS, randInt(1, 3)).map(project => {
    ...
    const failed  = Math.round(builds * (Math.random() < er ? jitter(0.15, 0.12, 0.01, 0.6) : jitter(0.05, 0.04, 0, 0.15)));
    return metricDoc(ts, "codebuild", "aws.codebuild", region, account,
      { ProjectName: project },
      {
...
const PIPELINES = ["api-deploy","frontend-pipeline","infra-pipeline","mobile-pipeline","ml-deploy"];
...
export function generateCodepipelineMetrics(ts, er) {
  ...
  return sample(PIPELINES, randInt(1, 2)).map(pipeline => {
    ...
    const failed = Math.round(execs * (Math.random() < er ? jitter(0.2, 0.15, 0.01, 0.7) : jitter(0.05, 0.04, 0, 0.15)));
    return metricDoc(ts, "codepipeline", "aws.codepipeline", region, account,
...
export function generateCodedeployMetrics(ts, er) {
  ...
  const failed  = Math.round(deploys * (Math.random() < er ? jitter(0.2, 0.15, 0.01, 0.6) : jitter(0.05, 0.04, 0, 0.15)));
  return [metricDoc(ts, "codedeploy", "aws.codedeploy", region, account,
...
export function generateAmplifyMetrics(ts, er) {
  ...
  return [metricDoc(ts, "amplify", "aws.amplify", region, account,
...
export function generateAutoscalingMetrics(ts, er) {
  ...
  return sample(ASG_NAMES, randInt(1, 3)).map(asg => {
    ...
    return metricDoc(ts, "autoscaling", "aws.autoscaling", region, account,
...
export function generateRoute53Metrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return sample(HOSTED_ZONES, randInt(1, 3)).map(zone => {
    return metricDoc(ts, "route53", "aws.route53", region, account,
      { HostedZoneId: `Z${randInt(10000000, 99999999)}`, Region: "Global" },
      {
        ...
        HealthCheckPercentageHealthy: stat(dp(Math.random() < er ? jitter(70, 20, 0, 95) : jitter(100, 0, 95, 100))),
...
export function generateBillingMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "billing", "aws.billing", region, account,
    { ServiceName: rand(["Amazon EC2","AWS Lambda","Amazon S3","Amazon RDS","Amazon DynamoDB","AWS CloudFront"]), LinkedAccount: account.id, Currency: "USD" },
    {
...
export function generateHealthMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "awshealth", "aws.awshealth", region, account,
...
export function generateInspectorMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "inspector", "aws.inspector", region, account,
    { Region: region },
    {
...
export function generateMacieMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "macie", "aws.macie", region, account,
    { Region: region },
    {
...
export function generateConfigMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "config", "aws.config", region, account,
    { Region: region },
    {
...
export function generateAccessanalyzerMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "accessanalyzer", "aws.accessanalyzer", region, account,
...
export function generateIotcoreMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "iot", "aws.iot", region, account,
...
export function generateWorkspacesMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "workspaces", "aws.workspaces", region, account,
...
export function generateConnectMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "connect", "aws.connect", region, account,
...
export function generateGameliftMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "gamelift", "aws.gamelift", region, account,
...
export function generateSecretsmanagerMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "secretsmanager", "aws.secretsmanager", region, account,
    { Region: region },
    {
...
export function generateAcmMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "acm", "aws.acm", region, account,
    { Region: region },
    {
...
export function generateCloudtrailMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "cloudtrail", "aws.cloudtrail", region, account,
...
export function generateAppsyncMetrics(ts, er) {
  ...
  return [metricDoc(ts, "appsync", "aws.appsync", region, account,
...
export function generateTrustedadvisorMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "trustedadvisor", "aws.trustedadvisor", region, account,
...
export function generateImagebuilderMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "imagebuilder", "aws.imagebuilder", region, account,
...
export function generateDevopsgouruMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "devopsguru", "aws.devopsguru", region, account,
    { Region: region },
    {
...
export function generatePinpointMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "pinpoint", "aws.pinpoint", region, account,
...
export function generateTransferfamilyMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "transfer", "aws.transfer", region, account,
...
export function generateLightsailMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "lightsail", "aws.lightsail", region, account,
    { InstanceName: rand(["web-server","wordpress","api-server","dev-box"]) },
    {
      CPUUtilization:            stat(dp(Math.random() < er ? jitter(75, 15, 50, 100) : jitter(25, 20, 1, 70))),
...
export function generateRamMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "ram", "aws.ram", region, account,
    { Region: region },
    {
...
export function generateDmsMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "dms", "aws.dms", region, account,
    { ReplicationTaskIdentifier: rand(["oracle-to-postgres","mysql-migration","sqlserver-to-aurora"]) },
    {
      ...
      CDCLatencySource:        stat(dp(Math.random() < er ? jitter(60, 50, 1, 3_600) : jitter(5, 4, 0, 30))),
      CDCLatencyTarget:        stat(dp(Math.random() < er ? jitter(120, 100, 1, 7_200) : jitter(10, 8, 0, 60))),
      FullLoadThroughputBandwidthSource: stat(dp(jitter(10_000_000, 8_000_000, 1_000_000, 500_000_000))),
...
export function generateCodecommitMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "codecommit", "aws.codecommit", region, account,
    { RepositoryName: rand(["api-service","frontend","infrastructure","mobile-app","ml-models"]) },
    {
...
export function generateCodeartifactMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "codeartifact", "aws.codeartifact", region, account,
    { Domain: rand(["mycompany","artifacts","packages"]), Repository: rand(["npm-store","maven-central","pypi-store"]) },
    {
...
const SH_STANDARDS = ["aws-foundational-security-best-practices","cis-aws-foundations-benchmark","pci-dss","nist-800-53"];

export function generateSecurityhubMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  const standard  = rand(SH_STANDARDS);
  const isFailing = Math.random() < er;
  return [metricDoc(ts, "securityhub", "aws.securityhub", region, account,
