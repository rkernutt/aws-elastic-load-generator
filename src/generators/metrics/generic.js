...
import { REGIONS, ACCOUNTS, rand, randInt, dp, stat, counter, metricDoc, pickCloudContext, jitter } from "./helpers.js";

// Generic metric shapes keyed by service family
const SERVICE_METRIC_TEMPLATES = {
  // IoT-like
  iot: [
...
const TEMPLATE_MAP = {
  iotgreengrass: "iot", iotanalytics: "analytics", iotevents: "iot",
  iotsitewise: "iot", iotdefender: "iot",
  controltower: "management", organizations: "management",
  servicecatalog: "management", servicequotas: "management",
  computeoptimizer: "management", budgets: "management",
  resiliencehub: "management", migrationhub: "management",
  identitycenter: "management", detective: "management",
  verifiedaccess: "management", securitylake: "management",
  mediaconvert: "analytics", medialive: "analytics",
  managedblockchain: "management", frauddetector: "analytics",
  locationservice: "management", appstream: "management",
  codeguru: "analytics", qbusiness: "analytics",
  comprehendmedical: "analytics", networkmanager: "management",
};

/**
 * Generic metric generator for services without a dedicated generator.
 * @param {string} serviceId - app service identifier (e.g. "iotgreengrass")
 * @param {string} dataset   - data_stream.dataset (e.g. "aws.iotgreengrass")
 * @returns {(ts: string, er: number) => Object[]}
 */
export function makeGenericGenerator(serviceId, dataset) {
  const templateKey = TEMPLATE_MAP[serviceId] ?? "default";
  const template    = SERVICE_METRIC_TEMPLATES[templateKey];
  const dimEntry    = template.find(t => t.dim);
  const metricsFn   = template.find(t => t.metrics)?.metrics ?? (() => ({ RequestCount: counter(randInt(0, 100_000)) }));

  return function genericMetricGenerator(ts, er) {
    ...
    return Array.from({ length: numDims }, (_, i) => {
      ...
      return metricDoc(ts, serviceId, dataset, region, account,
        { [dimKey]: dimVal },
        metricsFn(er),
      );
    });
