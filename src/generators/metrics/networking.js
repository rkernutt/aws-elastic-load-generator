...
const LB_NAMES = [
  "prod-alb","api-alb","web-alb","internal-alb","staging-alb",
  "prod-nlb","db-nlb","internal-nlb","tcp-nlb",
];

export function generateAlbMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return sample(LB_NAMES.filter(n => n.includes("alb")), randInt(1, 3)).map(name => {
    const req       = randInt(1_000, 500_000);
    const http5xx   = Math.round(req * (Math.random() < er ? jitter(0.05, 0.04, 0.001, 0.3) : jitter(0.002, 0.001, 0, 0.01)));
    const http4xx   = Math.round(req * jitter(0.015, 0.01, 0.001, 0.05));
    return metricDoc(ts, "elb", "aws.elb", region, account,
      { LoadBalancer: `app/${name}/abc123def456` },
      {
        ...
        TargetResponseTime:         stat(dp(jitter(0.05, 0.04, 0.001, 10)), { max: dp(jitter(5, 4, 0.5, 60)), min: dp(jitter(0.005, 0.003, 0.001, 0.05)) }),
...
export function generateNlbMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return sample(LB_NAMES.filter(n => n.includes("nlb")), randInt(1, 2)).map(name => {
    return metricDoc(ts, "elb", "aws.elb", region, account,
...
const APIGW_APIS   = ["users-api","orders-api","payments-api","products-api","auth-api","notifications-api","search-api"];
...
export function generateApigatewayMetrics(ts, er) {
  ...
  const err5  = Math.round(count * (Math.random() < er ? jitter(0.04, 0.03, 0.005, 0.25) : jitter(0.003, 0.002, 0, 0.01)));
  return [metricDoc(ts, "apigateway", "aws.apigateway_metrics", region, account,
    { ApiName: api, Stage: stage },
    {
      Count:              counter(count),
      "4XXError":         counter(err4),
      "5XXError":         counter(err5),
      Latency:            stat(dp(jitter(80, 60, 5, 10000)), { max: dp(jitter(2000, 1500, 200, 30000)), min: dp(jitter(10, 7, 1, 50)) }),
...
export function generateCloudfrontMetrics(ts, er) {
  ...
  return sample(CF_DISTRIBUTIONS, randInt(1, 3)).map(distId => {
    ...
    return metricDoc(ts, "cloudfront", "aws.cloudfront", region, account,
...
export function generateNatgatewayMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return Array.from({ length: randInt(1, 3) }, (_, i) => {
    const natId = `nat-${randInt(100000000, 999999999)}`;
    return metricDoc(ts, "natgateway", "aws.natgateway", region, account,
      { NatGatewayId: natId },
      {
...
export function generateTransitgatewayMetrics(ts, er) {
  ...
  return [metricDoc(ts, "transitgateway", "aws.transitgateway", region, account,
...
export function generateVpnMetrics(ts, er) {
  ...
  return Array.from({ length: randInt(1, 2) }, () => {
    ...
    return metricDoc(ts, "vpn", "aws.vpn", region, account,
      { VpnId: vpnId },
      {
...
export function generateNetworkfirewallMetrics(ts, er) {
  ...
  return [metricDoc(ts, "firewall", "aws.firewall", region, account,
...
export function generateGlobalacceleratorMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "globalaccelerator", "aws.globalaccelerator", region, account,
...
export function generateDirectconnectMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "directconnect", "aws.directconnect", region, account,
...
export function generateVpcMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "vpcflow", "aws.vpcflow", region, account,
    { Region: region },
    {
...
export function generatePrivatelinkMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "privatelink", "aws.privatelink", region, account,
