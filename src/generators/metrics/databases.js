...
import { REGIONS, ACCOUNTS, rand, randInt, randFloat, randId, dp, stat, counter, metricDoc, pickCloudContext, jitter, sample } from "./helpers.js";
...
function rdsMetrics(ts, er, instances, engine, dataset) {
  ...
  return sample(instances, randInt(2, 4)).map(inst => {
    ...
    return metricDoc(ts, engine ?? inst.engine, dataset, region, account,
      { DBInstanceIdentifier: inst.id, DBClusterIdentifier: inst.id.replace(/-\d+$/, "-cluster") },
      {
        ...
        DiskQueueDepth:               stat(dp(Math.random() < er ? jitter(5, 4, 0.5, 30) : jitter(0.2, 0.15, 0, 2))),
        ReplicaLag:                   stat(dp(Math.random() < er ? jitter(5000, 4000, 100, 60000) : jitter(200, 150, 0, 1000))),
...
export function generateRdsMetrics(ts, er)    { return rdsMetrics(ts, er, RDS_INSTANCES, "rds", "aws.rds"); }
export function generateAuroraMetrics(ts, er) { return rdsMetrics(ts, er, AURORA_INSTANCES, "aurora", "aws.rds"); }

// ─── DynamoDB ─────────────────────────────────────────────────────────────────

const DYNAMO_TABLES = [
  "orders","users","sessions","products","inventory","events","audit-log",
  "feature-flags","rate-limits","notifications","messages","carts","payments",
];
...
export function generateDynamodbMetrics(ts, er) {
  ...
  return sample(DYNAMO_TABLES, randInt(3, 7)).map(table => {
    ...
    return metricDoc(ts, "dynamodb", "aws.dynamodb", region, account,
      { TableName: table, Operation: rand(["GetItem","PutItem","Query","Scan","UpdateItem","DeleteItem"]) },
      {
...
export function generateElasticacheMetrics(ts, er) {
  ...
  return sample(CACHE_CLUSTERS, randInt(1, 3)).map(cluster => {
    ...
    return metricDoc(ts, "elasticache", "aws.elasticache", region, account,
      { CacheClusterId: cluster.id, CacheNodeId: "0001" },
      {
        ...
        ReplicationLag:           stat(dp(Math.random() < er ? jitter(500, 400, 10, 5000) : jitter(10, 8, 0, 100)) / 1000),
...
export function generateRedshiftMetrics(ts, er) {
  ...
  return sample(REDSHIFT_CLUSTERS, randInt(1, 2)).map(clusterId => {
    ...
    return metricDoc(ts, "redshift", "aws.redshift", region, account,
...
const S3_BUCKETS = [
  "prod-assets","data-lake","logs-archive","ml-datasets","backups",
  "static-content","media-uploads","reports","audit-logs","raw-events",
];
...
export function generateS3Metrics(ts, er) {
  ...
  return sample(S3_BUCKETS, randInt(3, 7)).map(bucket => {
    ...
    return metricDoc(ts, "s3", "aws.s3_daily_storage", region, account,
      { BucketName: bucket, StorageType: rand(["StandardStorage","IntelligentTieringFAStorage","StandardIAStorage","GlacierStorage"]) },
      {
...
export function generateDocdbMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "docdb", "aws.docdb", region, account,
    { DBClusterIdentifier: rand(["docdb-prod","docdb-staging","docdb-analytics"]), DBInstanceIdentifier: `docdb-instance-${randInt(1,3)}` },
    {
      CPUUtilization:           stat(dp(Math.random() < er ? jitter(75, 15, 50, 100) : jitter(25, 15, 5, 70))),
...
export function generateOpensearchMetrics(ts, er) {
  ...
  return [metricDoc(ts, "opensearch", "aws.opensearch", region, account,
    { DomainName: domain, ClientId: account.id },
    {
      CPUUtilization:              stat(dp(Math.random() < er ? jitter(80, 12, 60, 100) : jitter(35, 20, 5, 80))),
      JVMMemoryPressure:           stat(dp(Math.random() < er ? jitter(85, 10, 70, 98) : jitter(40, 20, 10, 75))),
...
export function generateNeptuneMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "neptune", "aws.neptune", region, account,
    { DBClusterIdentifier: rand(["graph-prod","knowledge-graph","recommendation-graph"]), DBInstanceIdentifier: "neptune-instance-1" },
    {
      CPUUtilization:           stat(dp(Math.random() < er ? jitter(75, 15, 50, 100) : jitter(30, 20, 5, 70))),
      DatabaseConnections:      counter(randInt(1, 100)),
...
export function generateKeyspacesMetrics(ts, er) {
  ...
  return [metricDoc(ts, "keyspaces", "aws.keyspaces", region, account,
...
export function generateMemorydbMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "memorydb", "aws.memorydb", region, account,
    { ClusterName: rand(["session-store","leaderboard","realtime-cache"]), NodeName: "cluster-0001-0001" },
    {
      CPUUtilization:    stat(dp(Math.random() < er ? jitter(70, 15, 50, 100) : jitter(20, 15, 1, 60))),
      FreeableMemory:    stat(dp(jitter(2_000_000_000, 1_000_000_000, 100_000_000, 8_000_000_000))),
      NetworkBytesIn:    counter(randInt(1_000_000, 5_000_000_000)),
      NetworkBytesOut:   counter(randInt(1_000_000, 5_000_000_000)),
...
export function generateEbsMetrics(ts, er) {
  ...
  return Array.from({ length: randInt(2, 6) }, () => {
    ...
    return metricDoc(ts, "ebs", "aws.ebs", region, account,
      { VolumeId: volId },
      {
...
export function generateEfsMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "efs", "aws.efs", region, account,
    { FileSystemId: `fs-${randId(8).toLowerCase()}` },
    {
      BurstCreditBalance:    stat(dp(Math.random() < er ? jitter(500_000_000, 400_000_000, 0, 2_000_000_000) : jitter(1_500_000_000_000, 500_000_000_000, 0, 2_300_000_000_000))),
      ...
      PercentIOLimit:        stat(dp(Math.random() < er ? jitter(80, 15, 50, 100) : jitter(20, 15, 0, 60))),
    }
  )];
}
...
export function generateTimestreamMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "timestream", "aws.timestream", region, account,
    { DatabaseName: rand(["iot-timeseries","metrics-db","telemetry"]), TableName: rand(["sensors","metrics","events","logs"]) },
    {
...
export function generateBackupMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "backup", "aws.backup", region, account,
...
export function generateFsxMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "fsx", "aws.fsx", region, account,
    { FileSystemId: `fs-${randId(8).toLowerCase()}` },
    {
      ...
      FreeStorageCapacity: stat(dp(jitter(100_000_000_000, 50_000_000_000, 1_000_000_000, 1_000_000_000_000))),
    }
  )];
}
...
export function generateStoragelensMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "s3_storage_lens", "aws.s3_storage_lens", region, account,
...
export function generateDatasyncMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "datasync", "aws.datasync", region, account,
...
export function generateStoragegatewayMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "storagegateway", "aws.storagegateway", region, account,
    { GatewayId: `sgw-${randId(8)}`, GatewayName: rand(["prod-gateway","backup-gw","file-gw"]) },
    {
      CacheHitPercent:     stat(dp(Math.random() < er ? jitter(40, 25, 0, 70) : jitter(90, 8, 70, 100))),
...
export function generateQldbMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "qldb", "aws.qldb", region, account,
