...
import { REGIONS, ACCOUNTS, rand, randInt, randFloat, randId, sample, jitter, dp, stat, counter, metricDoc, pickCloudContext } from "./helpers.js";

// ─── Lambda ───────────────────────────────────────────────────────────────────

const LAMBDA_NAMES = [
  "api-handler","auth-service","order-processor","notification-sender","image-resizer",
  "data-transformer","payment-processor","report-generator","webhook-handler","cache-warmer",
  "event-consumer","session-manager","file-processor","search-indexer","email-dispatcher",
  "audit-logger","cleanup-scheduler","token-validator","media-encoder","rate-limiter",
];
...
export function generateLambdaMetrics(ts, er) {
  ...
  return sample(LAMBDA_NAMES, randInt(3, 8)).map(name => {
    ...
    const errs  = Math.round(inv * (Math.random() < er ? jitter(0.08, 0.06, 0.01, 0.4) : jitter(0.005, 0.004, 0, 0.02)));
    const dur   = jitter(300, 250, 10, 15000);
    return metricDoc(ts, "lambda", "aws.lambda", region, account,
      { FunctionName: name, Resource: name },
      {
        Invocations:          counter(inv),
        Errors:               counter(errs),
        Duration:             stat(dp(dur), { max: dp(dur * jitter(3, 1.5, 1.5, 8)), min: dp(dur * jitter(0.3, 0.15, 0.05, 0.8)) }),
        Throttles:            counter(Math.round(inv * (Math.random() < 0.1 ? jitter(0.02, 0.015, 0, 0.1) : 0))),
...
const EC2_INSTANCE_TYPES = ["t3.micro","t3.small","t3.medium","t3.large","m5.large","m5.xlarge","m5.2xlarge","c5.large","c5.xlarge","r5.large"];
const EC2_AMI_PREFIXES   = ["ami-0a1b2c3d","ami-1b2c3d4e","ami-2c3d4e5f","ami-3d4e5f6a","ami-4e5f6a7b"];
...
export function generateEc2Metrics(ts, er) {
  ...
  return sample(EC2_INSTANCE_TYPES, randInt(3, 10)).map((itype, idx) => {
    ...
    return metricDoc(ts, "ec2", "aws.ec2", region, account,
      { InstanceId: instanceId, InstanceType: itype, AutoScalingGroupName: `asg-${["web","app","worker","api"][idx % 4]}-${region}` },
      {
...
const ECS_SERVICES  = ["web-frontend","api-gateway","auth-service","order-service","payment-service","notification-service","search-service","inventory-service"];
...
export function generateEcsMetrics(ts, er) {
  ...
  return sample(ECS_SERVICES, randInt(2, 5)).map(svc => {
    ...
    return metricDoc(ts, "ecs", "aws.ecs_metrics", region, account,
...
const EKS_NAMESPACES = ["default","kube-system","monitoring","production","staging","applications"];
...
export function generateEksMetrics(ts, er) {
  ...
  return sample(EKS_NAMESPACES, randInt(2, 4)).map(ns => {
    ...
    return metricDoc(ts, "kubernetes", "aws.kubernetes", region, account,
...
export function generateApprunnerMetrics(ts, er) {
  ...
  return sample(APPRUNNER_SERVICES, randInt(1, 3)).map(svc => {
    ...
    return metricDoc(ts, "apprunner", "aws.apprunner", region, account,
      { ServiceName: svc },
      {
        ...
        RequestLatency:   stat(dp(jitter(50, 40, 5, 5000)), { max: dp(jitter(500, 400, 100, 10000)), min: dp(jitter(5, 3, 1, 50)) }),
...
export function generateBatchMetrics(ts, er) {
  ...
  return sample(BATCH_QUEUES, randInt(2, 4)).map(q => {
    return metricDoc(ts, "batch", "aws.batch", region, account,
      { JobQueueName: q },
      {
        ...
        FailedJobCount:     counter(Math.round(randInt(0, 1000) * (Math.random() < er ? jitter(0.1, 0.08, 0, 0.5) : jitter(0.01, 0.008, 0, 0.05)))),
      }
    );
  });
}
...
export function generateElasticbeanstalkMetrics(ts, er) {
  ...
  return sample(EB_ENVS, randInt(1, 3)).map(env => {
    ...
    return metricDoc(ts, "elasticbeanstalk", "aws.elasticbeanstalk", region, account,
      { EnvironmentName: `myapp-${env}` },
      {
        ...
        ApplicationRequests5xx: counter(Math.random() < er ? randInt(1, Math.round(req * 0.08)) : 0),
...
const ECR_REPOS = ["app/frontend","app/backend","app/worker","infra/nginx","infra/fluentd","ml/inference"];

export function generateEcrMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return sample(ECR_REPOS, randInt(2, 5)).map(repo => {
    return metricDoc(ts, "ecr", "aws.ecr", region, account,
      { RepositoryName: repo },
      {
