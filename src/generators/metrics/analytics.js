...
import { REGIONS, ACCOUNTS, rand, randInt, randFloat, dp, stat, counter, metricDoc, pickCloudContext, jitter, sample } from "./helpers.js";

// ─── Glue ─────────────────────────────────────────────────────────────────────

const GLUE_JOBS = [
  "s3-to-redshift-etl","raw-data-transformer","data-quality-checker",
  "daily-aggregation","catalog-crawler","parquet-converter","ml-feature-prep",
];
...
export function generateGlueMetrics(ts, er) {
  ...
  return sample(GLUE_JOBS, randInt(1, 3)).map(job => {
    ...
    return metricDoc(ts, "glue", "aws.glue", region, account,
...
export function generateEmrMetrics(ts, er) {
  ...
  return [metricDoc(ts, "emr", "aws.emr_metrics", region, account,
    { JobFlowId: cluster.id, JobFlowName: cluster.name },
    {
      ...
      HDFSUtilization:           stat(dp(Math.random() < er ? jitter(85, 10, 60, 99) : jitter(40, 25, 5, 80))),
      ...
      YARNMemoryAvailablePercentage: stat(dp(Math.random() < er ? jitter(10, 8, 0, 30) : jitter(60, 25, 10, 95))),
    }
  )];
}
...
export function generateAthenaMetrics(ts, er) {
  ...
  return sample(ATHENA_WORKGROUPS, randInt(1, 3)).map(wg => {
    ...
    return metricDoc(ts, "athena", "aws.athena", region, account,
      { WorkGroup: wg },
      {
...
export function generateSagemakerMetrics(ts, er) {
  ...
  return sample(SAGEMAKER_ENDPOINTS, randInt(1, 3)).map(ep => {
    ...
    const modelErr    = Math.round(invocations * (Math.random() < er ? jitter(0.05, 0.04, 0.001, 0.3) : jitter(0.001, 0.0008, 0, 0.005)));
    return metricDoc(ts, "sagemaker", "aws.sagemaker", region, account,
      { EndpointName: ep.name, VariantName: "AllTraffic" },
      {
        ...
        CPUUtilization:                 stat(dp(Math.random() < er ? jitter(85, 10, 60, 100) : jitter(40, 25, 5, 80))),
        MemoryUtilization:              stat(dp(jitter(55, 25, 10, 95))),
        GPUUtilization:                 ep.instance.includes("g4dn") ? stat(dp(jitter(70, 20, 10, 100))) : undefined,
...
export function generateBedrockMetrics(ts, er) {
  ...
  return sample(BEDROCK_MODELS, randInt(1, 3)).map(modelId => {
    ...
    return metricDoc(ts, "bedrock", "aws.bedrock", region, account,
      { ModelId: modelId },
      {
        Invocations:              counter(invocations),
        InvocationLatency:        stat(dp(jitter(1500, 1200, 100, 30_000)), { max: dp(jitter(15_000, 10_000, 2_000, 120_000)) }),
...
const BEDROCK_AGENTS = ["customer-support-agent","research-assistant","code-reviewer","data-analyst"];
...
export function generateBedrockagentMetrics(ts, er) {
  ...
  return sample(BEDROCK_AGENTS, randInt(1, 2)).map(agentName => {
    ...
    return metricDoc(ts, "bedrockagent", "aws.bedrockagent", region, account,
      { AgentId: `AGT${Math.random().toString(36).substring(2, 12).toUpperCase()}`, AgentName: agentName },
      {
...
export function generateRekognitionMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  const ops = ["DetectLabels","DetectFaces","DetectText","RecognizeCelebrities","DetectModerationLabels"];
  return sample(ops, randInt(2, 4)).map(op => {
    const calls = randInt(0, 10_000);
    return metricDoc(ts, "rekognition", "aws.rekognition", region, account,
      { Operation: op },
      {
...
export function generateTranscribeMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "transcribe", "aws.transcribe", region, account,
    { Region: region },
    {
...
export function generateTranslateMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "translate", "aws.translate", region, account,
    { Operation: "TranslateText", SourceLanguageCode: rand(["en","es","fr","de","zh"]), TargetLanguageCode: rand(["fr","de","ja","pt","ar"]) },
    {
...
export function generatePollyMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "polly", "aws.polly", region, account,
...
export function generateComprehendMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  const ops = ["DetectSentiment","DetectEntities","DetectKeyPhrases","DetectLanguage","ClassifyDocument"];
  return sample(ops, randInt(2, 3)).map(op => {
    return metricDoc(ts, "comprehend", "aws.comprehend", region, account,
      { Operation: op },
      {
...
export function generateLexMetrics(ts, er) {
  ...
  return sample(bots, randInt(1, 2)).map(bot => {
    ...
    const missed = Math.round(reqs * (Math.random() < er ? jitter(0.2, 0.15, 0.05, 0.6) : jitter(0.05, 0.04, 0, 0.2)));
    return metricDoc(ts, "lex", "aws.lex", region, account,
...
export function generateForecastMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "forecast", "aws.forecast", region, account,
...
export function generatePersonalizeMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "personalize", "aws.personalize", region, account,
...
export function generateLookoutmetricsMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "lookoutmetrics", "aws.lookoutmetrics", region, account,
    { AnomalyDetectorName: rand(["sales-anomaly-detector","traffic-anomalies","fraud-detector"]) },
    {
...
export function generateTextractMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  const ops = ["DetectDocumentText","AnalyzeDocument","StartDocumentAnalysis","GetDocumentAnalysis"];
  return sample(ops, randInt(1, 3)).map(op => {
    return metricDoc(ts, "textract", "aws.textract", region, account,
      { Operation: op },
      {
...
export function generateLakeformationMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "lakeformation", "aws.lakeformation", region, account,
    { Region: region },
    {
...
export function generateQuicksightMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "quicksight", "aws.quicksight", region, account,
...
export function generateAppflowMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "appflow", "aws.appflow", region, account,
...
export function generateDatabrewMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "databrew", "aws.databrew", region, account,
...
export function generateXrayMetrics(ts, er) {
  const { region, account } = pickCloudContext(REGIONS, ACCOUNTS);
  return [metricDoc(ts, "xray", "aws.xray", region, account,
    { Region: region },
    {
...
export function generateSesMetrics(ts, er) {
  ...
  return [metricDoc(ts, "ses", "aws.ses", region, account,
    { Region: region },
    {
      Send:            counter(sent),
      Delivery:        counter(Math.round(sent * jitter(0.96, 0.03, 0.8, 1))),
      Bounce:          counter(Math.round(sent * (Math.random() < er ? jitter(0.05, 0.04, 0.001, 0.2) : jitter(0.003, 0.002, 0, 0.01)))),
