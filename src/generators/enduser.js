...
function generateWorkSpacesLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"workspaces"}},
    ...
    "event":{action,outcome:isErr?"failure":"success",category:["host","session"],dataset:"aws.workspaces",provider:"workspaces.amazonaws.com",duration:randInt(100,isErr?10000:5000)*1e6},
...
function generateConnectLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"connect"}},
    ...
    "event":{duration:dur*1e9,outcome:isErr?"failure":"success",category:["host","session"],dataset:"aws.connect",provider:"connect.amazonaws.com"},
...
function generateAppStreamLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"appstream"}},"aws":{appstream:{stack_name:stack,fleet_name:fleet,user_id:user,session_id:randId(36).toLowerCase(),event_type:event,application_name:event.includes("APP")?rand(["Notepad++","MATLAB","AutoCAD","Chrome","VS Code","Tableau"]):null,instance_type:rand(["stream.standard.medium","stream.compute.large","stream.memory.xlarge"]),session_duration_minutes:event.includes("ENDED")?randInt(1,480):null,storage_connector:rand([null,"HomeFolder","OneDrive","GoogleDrive"]),idle_disconnect_timeout_minutes:rand([15,30,60]),max_user_duration_hours:rand([2,4,8,12]),error_code:isErr?rand(["FLEET_CAPACITY_EXCEEDED","IAM_SERVICE_ROLE_ERROR","USER_NOT_AUTHORIZED"]):null}},"user":{name:user},"event":{outcome:isErr?"failure":"success",category:["host","session"],dataset:"aws.appstream",provider:"appstream2.amazonaws.com",duration:randInt(100,isErr?30000:5000)*1e6},"message":isErr?`AppStream ${stack} ${event} FAILED: ${rand(["Fleet at capacity","IAM role error","User not authorized"])}:`:`AppStream ${event}: ${user} [${stack}]`,"log":{level:isErr?"error":"info"},...(isErr?{error:{code:rand(["FLEET_CAPACITY_EXCEEDED","IAM_SERVICE_ROLE_ERROR","USER_NOT_AUTHORIZED"]),message:"AppStream operation failed",type:"session"}}:{}) };
}
...
function generateGameLiftLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"gamelift"}},"aws":{
    ...
    }}},"event":{outcome:isErr?"failure":"success",category:["process","network"],dataset:"aws.gamelift",provider:"gamelift.amazonaws.com",duration:randInt(100,isErr?30000:7200000)*1e6},"message":isErr?`GameLift ${fleet} ${event} FAILED: ${rand(["Fleet at capacity","Invalid session","Instance unavailable"])}:`:`GameLift ${fleet}: ${event}`,"log":{level:isErr?"error":event.includes("TimedOut")?"warn":"info"},...(isErr?{error:{code:rand(["InvalidFleetStatus","FleetCapacityExceeded","InvalidGameSession"]),message:"GameLift operation failed",type:"session"}}:{}) };
}
...
function generateSesLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"ses"}},
    "aws":{
      dimensions:{ "ses:configuration-set":configSet||"default", "ses:from-domain":fromDomain, "ses:to-country":toCountry, "ses:to-isp":toIsp },
      ses:{message_id:msgId,event_type:event,from_address:from,destination:to,configuration_set:configSet,bounce:{bounce_type:bounceType,bounce_sub_type:bounceSubType},complaint:{feedback_type:event==="Complaint"?rand(["abuse","fraud","virus","not-spam"]):null},sending_account_id:`${acct.id}`,delivery:{recipients:[to],timestamp:ts,processing_time_ms:randInt(50,3000),smtp_response:isErr?null:"250 2.0.0 OK"},
      metrics:{
        Send:{ sum:1 },
...
function generateMediaLiveLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"medialive"}},
    ...
    "event":{outcome:isErr?"failure":"success",category:["process"],dataset:"aws.medialive",provider:"medialive.amazonaws.com",duration:randInt(1000,isErr?30000:10000)*1e6},
