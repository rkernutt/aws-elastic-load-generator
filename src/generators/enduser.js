...
function generateWorkSpacesLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"workspaces"}},
    ...
    "event":{action,outcome:isErr?"failure":"success",category:"session",dataset:"aws.workspaces",provider:"workspaces.amazonaws.com",duration:randInt(100,isErr?10000:5000)*1e6},
...
function generateAppStreamLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"appstream"}},"aws":{appstream:{stack_name:stack,fleet_name:fleet,user_id:user,session_id:randId(36).toLowerCase(),event_type:event,application_name:event.includes("APP")?rand(["Notepad++","MATLAB","AutoCAD","Chrome","VS Code","Tableau"]):null,instance_type:rand(["stream.standard.medium","stream.compute.large","stream.memory.xlarge"]),session_duration_minutes:event.includes("ENDED")?randInt(1,480):null,storage_connector:rand([null,"HomeFolder","OneDrive","GoogleDrive"]),idle_disconnect_timeout_minutes:rand([15,30,60]),max_user_duration_hours:rand([2,4,8,12]),error_code:isErr?rand(["FLEET_CAPACITY_EXCEEDED","IAM_SERVICE_ROLE_ERROR","USER_NOT_AUTHORIZED"]):null}},"user":{name:user},"event":{outcome:isErr?"failure":"success",category:"session",dataset:"aws.appstream",provider:"appstream2.amazonaws.com",duration:randInt(100,isErr?30000:5000)*1e6},"message":isErr?`AppStream ${stack} ${event} FAILED: ${rand(["Fleet at capacity","IAM role error","User not authorized"])}:`:`AppStream ${event}: ${user} [${stack}]`,"log":{level:isErr?"error":"info"},...(isErr?{error:{code:rand(["FLEET_CAPACITY_EXCEEDED","IAM_SERVICE_ROLE_ERROR","USER_NOT_AUTHORIZED"]),message:"AppStream operation failed",type:"session"}}:{}) };
}
