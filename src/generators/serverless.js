...
function generateLambdaLog(ts, er) {
  ...
  return {
    ...
    "aws": {
      dimensions: { FunctionName:fn, Resource:`${fn}:$LATEST`, ExecutedVersion:"$LATEST", ...(hasMapping ? { EventSourceMappingUUID: randUUID() } : {}) },
      ...
      lambda: {
        ...
        ...(isErr ? { error: { message: rand(["Unhandled exception","DB connection refused","Timeout after 30000ms"]), stack_trace: `Error: Unhandled exception\n    at handler (index.js:${randInt(10,200)}:${randInt(5,30)})\n    at Runtime.handler` } } : {}),
        metrics: {
