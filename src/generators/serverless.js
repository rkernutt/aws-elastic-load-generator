...
function generateApiGatewayLog(ts, er) {
  ...
  const integrationLat = Math.floor(lat * parseFloat(randFloat(0.55, 0.9)));
    ...
    aws: {
      appsync: {
        ...
        metrics: {
          ...
          Latency: {
            avg: dur,
            p99: parseFloat((dur * parseFloat(randFloat(1.5, 4.0))).toFixed(1)),
          },
        },
      },
    },
...
function generateFargateLog(ts, er) {
  ...
  const memPct = parseFloat(
    randFloat(10, Math.min(fargateMemReservation, level === "error" ? 99 : 80))
  );
  const svc = task;
  return {
    "@timestamp": ts,
