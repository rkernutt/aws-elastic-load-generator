    ...
    "aws": {
      ...
      kinesis: {
        metrics: {
          ...
          GetRecords_IteratorAgeMilliseconds: { avg: isErr?randInt(10000,3600000):randInt(0,1000) },
          GetRecords_Latency: { avg: randInt(1,50) },
          GetRecords_Records: { sum: randInt(1,1000) },
...
function generateSnsLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"sns"}},
    "aws":{dimensions:{TopicName:topic},sns:{metrics:{
        ...
        SMSMonthToDateSpentUSD: { sum: protocol==="sms"?parseFloat(randFloat(0,50)):0 },
      }}},
