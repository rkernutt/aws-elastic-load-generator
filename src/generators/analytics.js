...
function generateEmrLog(ts, er) {
  ...
  const sparkShuffleMsg = () =>
    `Shuffle read: ${parseFloat(randFloat(0.1, 5.2)).toFixed(1)} GB, Shuffle write: ${parseFloat(randFloat(0.1, 4.8)).toFixed(1)} GB`;
  const infoBase = [
    "Job run started",
    ...
    aws: {
      ...
      elasticmapreduce: {
        metrics: {
          ...
          YARNMemoryAvailablePercentage: { avg: parseFloat(randFloat(5, 80)) },
          ...
          CapacityRemainingGB: { avg: parseFloat(randFloat(10, 5000)) },
          ...
          ContainerPendingRatio: {
            avg:
              level === "error" ? parseFloat(randFloat(0.5, 5.0)) : parseFloat(randFloat(0, 0.5)),
          },
...
function generateAthenaLog(ts, er) {
  ...
  const dur = parseFloat(randFloat(0.5, isErr ? 300 : 60));
