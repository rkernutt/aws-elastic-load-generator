...
function generateIotCoreLog(ts, er) {
  ...
  return {
    "@timestamp": ts,
    "cloud": { provider:"aws", region, account:{ id:acct.id, name:acct.name }, service:{ name:"iotcore" } },
    "aws": {
...
function generateIotGreengrassLog(ts, er) {
  ...
  return {
    "@timestamp": ts,
    "cloud": { provider:"aws", region, account:{ id:acct.id, name:acct.name }, service:{ name:"greengrass" } },
    "aws": {
      ...
      greengrass: {
        ...
        metrics: {
          ...
          StreamManagerMemoryUsage:      { avg: parseFloat(randFloat(10,512)) },
          StreamManagerBytesAppended:    { sum: randInt(0,10485760) },
          NucleusRestartCount:           { sum: isErr ? randInt(1,5) : 0 },
          DeploymentQueueSize:           { avg: parseFloat(randFloat(0,20)) },
        },
      },
    },
    "event": {
...
function generateIotAnalyticsLog(ts, er) {
  ...
  return {
    "@timestamp": ts,
    "cloud": { provider:"aws", region, account:{ id:acct.id, name:acct.name }, service:{ name:"iotanalytics" } },
    "aws": {
...
function generateIotDefenderLog(ts, er) {
  ...
  return {
    "@timestamp": ts,
    "cloud": { provider:"aws", region, account:{ id:acct.id, name:acct.name }, service:{ name:"iotdefender" } },
    "aws": {
      ...
      iotdefender: {
        ...
        security_profile_name: rand(["baseline-security-profile","factory-floor-profile","critical-devices"]),
        ...
        error_code: isErr ? rand(["ResourceNotFoundException","ThrottlingException","InternalFailureException"]) : null,
      },
    },
    "event": {
      kind: "alert",
    ...
    ...(isErr ? { error: { code: rand(["ResourceNotFoundException","ThrottlingException","InternalFailureException"]), message: "IoT Defender audit error", type: "iot" } } : {}),
  };
}
...
function generateIotEventsLog(ts, er) {
  ...
  return {
    "@timestamp": ts,
    "cloud": { provider:"aws", region, account:{ id:acct.id, name:acct.name }, service:{ name:"iotevents" } },
    "aws": {
      ...
      iotevents: {
        ...
        condition_expression: rand([null,"$input.SensorInput.temperature > 85","$input.data.value < threshold"]),
        error_code: isErr ? rand(["ResourceNotFound","ThrottlingException","InvalidRequestException"]) : null,
      },
    },
    "event": {
      outcome: isErr ? "failure" : "success",
    ...
    ...(isErr ? { error: { code: rand(["ResourceNotFound","ThrottlingException","InvalidRequestException"]), message: "IoT Events error", type: "iot" } } : {}),
  };
}
...
function generateIotSiteWiseLog(ts, er) {
  ...
  return {
    "@timestamp": ts,
    "cloud": { provider:"aws", region, account:{ id:acct.id, name:acct.name }, service:{ name:"iotsitewise" } },
    "aws": {
      ...
      iotsitewise: {
        ...
        error: isErr ? rand(["BatchPutAssetPropertyValue failed","Property not found","Quota exceeded"]) : null,
        metrics: {
          GatewayDataStreamPartitionCount: { avg: parseFloat(randFloat(1,64)) },
    ...
    ...(isErr ? { error: { code: "SiteWiseError", message: "IoT SiteWise quality/error", type: "iot" } } : {}),
  };
}
