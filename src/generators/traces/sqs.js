...
function buildSpan(traceId, txId, parentId, ts, sdkKey, isErr, spanOffset, spanDuration, labels) {
  ...
  const shapes = {
    dynamodb: {
      ...
      name: () => `DynamoDB.${rand(["GetItem","PutItem","Query","UpdateItem","BatchWriteItem","Scan"])}`,
      action: () => rand(["GetItem","PutItem","Query","UpdateItem","BatchWriteItem","Scan"]),
      db: () => ({ type: "nosql", statement: `${rand(["PutItem","UpdateItem","BatchWriteItem"])} ${rand(["orders","users","inventory","audit_events","payments"])}` }),
      dest: "dynamodb",
    },
    s3: {
