...
function buildInvokeEndpointSpan(traceId, txId, parentId, ts, cfg, account, isErr, spanOffsetMs) {
  const id = newSpanId();
...
export function generateSageMakerTrace(ts, er) {
  ...
  if (includePreProcess) {
    ...
    spans.push(buildS3PreProcessSpan(
      traceId, txId, txId, ts,
      account, false, spanOffsetMs,
    ));
  ...
  spans.push(buildInvokeEndpointSpan(
    traceId, txId, txId, ts,
    cfg, account, inferenceIsErr, spanOffsetMs,
  ));
  ...
  if (includePostProcess) {
    ...
    spans.push(buildDynamoPostProcessSpan(
      traceId, txId, txId, ts,
      postIsErr, spanOffsetMs,
    ));
  }
