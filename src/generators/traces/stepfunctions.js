...
function toKebabCase(name) {
  return name.replace(/([A-Z])/g, (m, c, i) => (i === 0 ? c : "-" + c).toLowerCase()).toLowerCase();
}
...
export function generateStepFunctionsTrace(ts, er) {
  ...
  const executedStates = isErr && cfg.failState
    ? [...cfg.states.slice(0, -1), cfg.failState]
    : cfg.states;
