...
const SPARK_VERSIONS = { "7.1.0": "3.5.1", "7.0.0": "3.5.0", "6.15.0": "3.4.1", "6.14.0": "3.4.0", "6.13.0": "3.3.2" };
...
export function generateEmrTrace(ts, er) {
  ...
  const baseDurationMs  = cfg.jobType === "streaming_sql" ? randInt(30,120) * 60 * 1000 : randInt(3,25) * 60 * 1000;
  const totalUs         = baseDurationMs * 1000;

  const svcBlock = serviceBlock(
    cfg.appName, env, "java",
    "Spark",
    "OpenJDK", "21.0.3",
  );
  // Override framework to include Spark version
  svcBlock.framework = { name: "Spark", version: sparkVer };
