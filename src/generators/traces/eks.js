...
const SERVICE_CONFIGS = [
  ...
  },
  {
    ...
    framework: "FastAPI",
    txType: "request",
    http: { method: "GET", paths: ["/recommend/{userId}", "/recommend/trending", "/recommend/similar/{itemId}"] },
  ...
  },
  {
    ...
    framework: "FastAPI",
    txType: "request",
    http: { method: "GET", paths: ["/users/{id}", "/users/{id}/preferences", "/users/{id}/activity"] },
...
function buildDownstreamSpan(traceId, txId, ts, spanKey, isErr, offsetMs) {
  ...
  const shapes = {
    ...
    redis: {
      ...
      name: () => `Redis ${rand(["GET", "SET", "HGET", "HSET", "ZADD", "ZRANGE", "DEL", "EXPIRE"])}`,
      db: () => ({ type: "redis", statement: rand([
        "GET session:{userId}",
        "SET cache:{key} {value}",
        "HGETALL user:{id}",
    ...
    dynamodb: {
      ...
      name: () => `DynamoDB.${rand(["GetItem", "PutItem", "Query", "UpdateItem", "BatchWriteItem"])}`,
      db: () => ({ type: "nosql", statement: `${rand(["GetItem", "Query", "Scan"])} ${rand(["events", "recommendations", "profiles", "sessions"])}` }),
      dest: "dynamodb",
    },
    elasticsearch: {
      type: "db", subtype: "elasticsearch",
      name: () => `Elasticsearch ${rand(["search", "index", "get", "bulk"])}`,
      db: () => ({ type: "elasticsearch", statement: rand([
        '{"query":{"match":{"title":"$1"}}}',
    ...
    "kafka-produce": {
      type: "messaging", subtype: "kafka",
      name: () => `Kafka ${rand(["PRODUCE", "SEND"])} to ${rand(["order-events", "payment-events", "notification-events"])}`,
      db: null,
      dest: "kafka",
    },
    sns: {
    ...
    "http-ml": {
      ...
      name: () => `GET ${rand(["ml-service.internal", "inference.internal", "model-api.internal"])}/predict`,
      db: null,
    ...
    "http-downstream": {
      type: "external", subtype: "http",
      name: () => {
        const svc = rand(["payment-service", "inventory-service", "user-profile-service", "search-service", "notification-service"]);
        const method = rand(["GET", "POST"]);
        return `${method} ${svc}.production.svc.cluster.local`;
      },
      db: null,
      dest: rand(["payment-service.production.svc", "inventory-service.production.svc", "user-profile-service.production.svc"]),
    },
  };
...
export function generateEksTrace(ts, er) {
  ...
  const svcBlock = serviceBlock(
    ...
    cfg.runtime, cfg.runtimeVersion,
  );
  ...
  for (let i = 0; i < cfg.spans.length; i++) {
    ...
    const spanDoc   = buildDownstreamSpan(traceId, txId, ts, spanKey, spanIsErr, offsetMs);
