...
const FUNCTION_CONFIGS = [
  { name: "order-processor",        runtime: "python3.12", trigger: "sqs",       sdkCalls: ["sqs","dynamodb","sns"] },
  { name: "user-auth-handler",      runtime: "nodejs20.x", trigger: "http",      sdkCalls: ["cognito","dynamodb","secretsmanager"] },
  { name: "image-resizer",          runtime: "nodejs18.x", trigger: "s3",        sdkCalls: ["s3","rekognition"] },
  { name: "data-pipeline-ingest",   runtime: "python3.11", trigger: "kinesis",   sdkCalls: ["s3","dynamodb","kinesis"] },
  { name: "notification-sender",    runtime: "python3.12", trigger: "scheduled", sdkCalls: ["ses","sns","dynamodb"] },
  { name: "api-backend",            runtime: "nodejs20.x", trigger: "http",      sdkCalls: ["dynamodb","s3","elasticache"] },
  { name: "etl-transformer",        runtime: "java21",     trigger: "s3",        sdkCalls: ["s3","dynamodb","glue"] },
  { name: "fraud-scorer",           runtime: "python3.11", trigger: "sqs",       sdkCalls: ["dynamodb","sagemaker","sns"] },
  { name: "inventory-updater",      runtime: "python3.12", trigger: "eventbridge", sdkCalls: ["dynamodb","sqs","sns"] },
  { name: "report-generator",       runtime: "java21",     trigger: "scheduled", sdkCalls: ["s3","rds","ses"] },
  { name: "stream-processor",       runtime: "nodejs20.x", trigger: "kinesis",   sdkCalls: ["dynamodb","kinesis","firehose"] },
  { name: "webhook-dispatcher",     runtime: "python3.12", trigger: "http",      sdkCalls: ["sqs","secretsmanager","dynamodb"] },
];

const RUNTIME_LANG = {
  "python3.11": "python", "python3.12": "python",
  "nodejs18.x": "nodejs", "nodejs20.x": "nodejs",
  "java21": "java", "go1.x": "go",
};

const RUNTIME_VERSION = {
  "python3.11": "3.11.9", "python3.12": "3.12.3",
  "nodejs18.x": "18.20.4", "nodejs20.x": "20.15.1",
  "java21": "21.0.3", "go1.x": "1.22.5",
};
...
function buildSpan(traceId, txId, parentId, ts, sdkKey, isErr, spanOffset, spanDuration) {
  ...
  const shapes = {
    ...
    s3: {
      ...
      name: () => `S3.${rand(["GetObject","PutObject","DeleteObject","ListObjectsV2","CopyObject"])}`,
...
export function generateLambdaTrace(ts, er) {
  ...
  const triggerTypeMap = {
    http: "other", sqs: "pubsub", kinesis: "pubsub",
    s3: "datastore", scheduled: "timer", eventbridge: "pubsub",
  };

  const svcBlock = serviceBlock(
    cfg.name, env, lang,
    "AWS Lambda",
    cfg.runtime, RUNTIME_VERSION[cfg.runtime] || "unknown",
  );
