...
const CONSUMER_CONFIGS = [
  ...
  },
  {
    ...
    description:  "Rolls up time-series metrics into DynamoDB and publishes aggregates to CloudWatch",
  },
  {
...
function buildSpan(traceId, txId, parentId, ts, sdkKey, isErr, spanOffset, spanDuration, labels) {
  ...
  const shapes = {
    dynamodb: {
      ...
      db: () => ({ type: "nosql", statement: `${rand(["BatchWriteItem","PutItem","UpdateItem"])} ${rand(["metrics","sessions","iot_readings","aggregations","checkpoints"])}` }),
      dest: "dynamodb",
    },
    s3: {
...
export function generateKinesisTrace(ts, er) {
  ...
  const runtimeName    = lang === "python" ? `python3.${rand([11,12])}` :
                         lang === "nodejs"  ? `nodejs${rand([18,20])}.x` : "java21";
  const runtimeVersion = lang === "python" ? `3.${rand([11,12])}.${randInt(0,9)}` :
                         lang === "nodejs"  ? `${rand([18,20])}.${randInt(0,20)}.${randInt(0,5)}` : "21.0.3";

  const svcBlock = serviceBlock(
    cfg.serviceName, env, lang,
    "AWS Kinesis",
  ...
  for (const sdkKey of cfg.sdkCalls) {
    ...
    spans.push(buildSpan(traceId, txId, txId, ts, sdkKey, spanIsErr, spanOffset, spanUs, sharedLabels));
