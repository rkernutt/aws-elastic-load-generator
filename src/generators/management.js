...
function generateCloudFormationLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"cloudformation"}},
    ...
    "event":{outcome:isErr?"failure":"success",category:["configuration","process"],dataset:"aws.cloudformation",provider:"cloudformation.amazonaws.com",duration:randInt(30,isErr?3600:600)*1e9},
    "message":message,
...
function generateSsmLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"systemsmanager"}},
    ...
    "event":{outcome:isErr?"failure":"success",category:["process","configuration","host"],dataset:"aws.ssm",provider:"ssm.amazonaws.com",duration:randInt(1,isErr?300:30)*1e9},
    "message":message,
...
function generateCloudWatchAlarmsLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"cloudwatchalarms"}},
    ...
    "event":{kind:"alert",outcome:alarmState==="OK"?"success":"failure",category:["configuration","process"],dataset:"aws.cloudwatch",provider:"monitoring.amazonaws.com",duration:rand([60,300,3600])*1e9},
...
function generateTrustedAdvisorLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"trustedadvisor"}},
    ...
    "event":{kind:"alert",outcome:isFinding?"failure":"success",category:["configuration","vulnerability"],dataset:"aws.trustedadvisor",provider:"trustedadvisor.amazonaws.com",duration:randInt(5,60)*1e9},
...
function generateControlTowerLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"controltower"}},
    ...
    "event":{outcome:isErr?"failure":"success",category:["configuration","iam"],dataset:"aws.controltower",provider:"controltower.amazonaws.com",duration:randInt(30,isErr?1800:600)*1e9},
...
function generateOrganizationsLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region:"us-east-1",account:{id:acct.id,name:acct.name},service:{name:"organizations"}},
    ...
    "event":{action,outcome:isErr?"failure":"success",category:["configuration","iam"],dataset:"aws.organizations",provider:"organizations.amazonaws.com",duration:randInt(100,isErr?5000:2000)*1e6},
...
function generateBillingLog(ts, er) {
  ...
  return {
    ...
    "event": { outcome: isErr ? "failure" : "success", category: ["configuration","process"], dataset: "aws.billing", provider: "ce.amazonaws.com", duration: randInt(100, 5000)*1e6 },
...
function generateDmsLog(ts, er) {
  ...
  return { "@timestamp":ts,"cloud":{provider:"aws",region,account:{id:acct.id,name:acct.name},service:{name:"dms"}},
    "aws":{
      ...
      dms:{replication_task_id:taskName,migration_type:migrationType,
    ...
    "event":{outcome:isErr?"failure":"success",category:["database","process"],dataset:"aws.dms",provider:"dms.amazonaws.com",duration:randInt(60,isErr?86400:28800)*1e9},
