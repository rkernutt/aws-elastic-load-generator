    ...
    aws: {
      trustedadvisor: {
        ...
        estimated_monthly_savings:
          cat === "cost_optimizing" && isFinding ? parseFloat(randFloat(10, 5000)) : null,
...
function generateServiceQuotasLog(ts, er) {
  ...
  const current = isErr
    ? Math.floor(limit * parseFloat(randFloat(0.9, 1.1)))
    : Math.floor(limit * parseFloat(randFloat(0.5, 0.89)));
  return {
    "@timestamp": ts,
    cloud: {
      provider: "aws",
  ...
  return {
    ...
    aws: {
      computeoptimizer: {
        ...
        estimated_monthly_savings_percent: saving > 0 ? parseFloat(randFloat(10, 60)) : 0,
        lookback_period_days: rand([14, 32, 93]),
        utilization: {
          cpu_max: parseFloat(randFloat(5, 95)),
          memory_max: parseFloat(randFloat(10, 95)),
        },
...
function generateBudgetsLog(ts, er) {
  ...
  const limit = parseFloat(randFloat(100, 10000));
  const actual = isErr
    ? limit * (1 + parseFloat(randFloat(0.05, 0.5)))
    : limit * parseFloat(randFloat(0.3, 0.95));
  ...
  return {
    ...
    aws: {
      budgets: {
        ...
        forecasted_spend: parseFloat((actual * parseFloat(randFloat(0.9, 1.4))).toFixed(2)),
